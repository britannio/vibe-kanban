...
| Tool | Purpose | Required Parameters | Optional Parameters | Returns |
|------|---------|-------------------|-------------------|---------|
| `list_tasks` | List tasks in a project | `project_id` (UUID) | `status` (todo\|in-progress\|in-review\|done\|cancelled)<br/>`limit` (number, default: 50) | List of tasks with execution state |
| `create_task` | Create a new task | `project_id` (UUID)<br/>`title` (string) | `description` (string) | Created task ID and confirmation |
| `get_task` | Get task details | `task_id` (UUID) | None | Full task information |
| `update_task` | Update task details | `task_id` (UUID) | `title` (string)<br/>`description` (string)<br/>`status` (string) | Updated task information |
| `delete_task` | Delete a task | `task_id` (UUID) | None | Deletion confirmation |

### Task Execution

| Tool | Purpose | Required Parameters | Optional Parameters | Returns |
|------|---------|-------------------|-------------------|---------|
| `start_task_attempt` | Start working on a task with a coding agent | `task_id` (UUID)<br/>`executor` (string)<br/>`base_branch` (string) | `variant` (string) | Attempt ID and confirmation |

<Info>
The `start_task_attempt` tool is a newly added feature that allows you to programmatically start task attempts with specific coding agents directly from MCP clients.
</Info>
...
When filtering or updating tasks, use these kebab-case status values:

- `todo` - Task not started
- `in-progress` - Task currently being worked on
- `in-review` - Task pending review
- `done` - Task completed
...
The coding agent will use the `create_task` tool to break this down into individual tasks and add them to your Vibe Kanban project automatically.
...
The coding agent will use the `start_task_attempt` tool with parameters like:
```json
{
