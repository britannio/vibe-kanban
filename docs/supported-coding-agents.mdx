...
Choose a provider (e.g., `openrouter`, `deepseek`, etc.) and enter the required API key(s) and settings for that provider.
</Step>
...
<Note>
Not all models support web search, thinking, or images. Choose models accordingly in the CCR UI.
</Note>
</Step>
</Steps>
...
Open the "Coding Agent Configurations" page.
</Step>

<Step title="Add CCR configuration">
Add a new configuration for the Claude Code agent (or edit an existing one).
</Step>

<Step title="Enable CCR">
Enable the `claude_code_router` checkbox and optionally set a model string to target a specific CCR provider/model.
</Step>
</Steps>

See the [Agent Profiles & Variants](/user-guide/agent-configurations) guide for managing agent configurations.
