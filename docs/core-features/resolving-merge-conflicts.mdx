...
```mermaid
stateDiagram-v2
    [*] --> Mergeable: Task completed
    Mergeable --> Behind: Base branch advances
    Behind --> Mergeable: Rebase succeeds
    Behind --> RebaseConflicts: Rebase conflicts
    
    state RebaseConflicts {
        [*] --> ConflictDetected
        ConflictDetected --> ManualEdit: Open in Editor
        ConflictDetected --> AutoResolve: Insert Instructions
        ConflictDetected --> AbortRebase: Abort
        ManualEdit --> Resolved: git rebase --continue
        AutoResolve --> Resolved: Agent resolves
        AbortRebase --> Behind: Return to previous state
        Resolved --> [*]
    }
    
    RebaseConflicts --> Mergeable: All conflicts resolved
    Mergeable --> [*]: Merge task
    
    classDef ready fill:#d4edda,stroke:#28a745,color:#000
    classDef behind fill:#fff3cd,stroke:#ffc107,color:#000
    classDef conflict fill:#f8d7da,stroke:#dc3545,color:#000
    
    class Mergeable ready
    class Behind behind
    class RebaseConflicts,ConflictDetected,ManualEdit,AutoResolve conflict
```

**Three Core States:**
- **Mergeable** - Ready to merge, no conflicts
- **Behind** - Base branch advanced, rebase needed  
- **Rebase Conflicts** - Conflicts detected during rebase
