...
You can learn more about task templates on the [Task Templates](/user-guide/creating-task-templates) page.

## Agent Settings (Profiles & Variants)

Define and customize agent variants under Settings → Agents. Variants let you maintain multiple configurations for the same agent (for example a CLAUDE_CODE "PLAN" variant). See the [Agent Profiles & Variants](/user-guide/agent-configurations) guide for details and examples.
