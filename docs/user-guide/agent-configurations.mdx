...
MCP (Model Context Protocol) servers are configured per agent under Settings → MCP Servers. This is separate from profiles but complements them. 

For configuring MCP servers within Vibe Kanban for your coding agents, see the [MCP Server Configuration](/user-guide/mcp-server-configuration) guide. If you want to connect external MCP clients to Vibe Kanban's own MCP server, see the [Vibe Kanban MCP Server](/user-guide/vibe-kanban-mcp-server) guide.
